...
function createRiskReturnScatter() {
    ...
    const volatilities = fundNames.map(fund => fundData[fund].annualizedVolatility);
...
function createPortfolioAllocationChart() {
    ...
    const colorArray = assetClasses.map(assetClass => colors[assetClass] || 'rgb(156, 163, 175)');
    ...
    // Create chart
    Plotly.newPlot('portfolio-allocation-chart', data, layout, {responsive: true});
}
...
function createEfficientFrontierChart() {
    ...
    const fundVolatilities = fundNames.map(fund => fundData[fund].annualizedVolatility);
    ...
    const data = [
        ...
        // Capital Market Line
        {
            ...
            y: [0.03, 0.03 + frontierPoints.maxSharpeRatio * frontierPoints.maxSharpeVolatility * 1.5],
            mode: 'lines',
    ...
    // Create scatter plot
    Plotly.newPlot('efficient-frontier-chart', data, layout, {responsive: true});
}
...
function createProjectionChart(scenario) {
    ...
    // Get portfolio stats
    const portfolioReturn = appState.optimalPortfolio.stats.return * scenarioInfo.returnMultiplier;
    const portfolioVolatility = appState.optimalPortfolio.stats.volatility * scenarioInfo.volatilityMultiplier;
    ...
    const targetMetProbability = ((finalExpected - finalLower) / (finalUpper - finalLower)) * 100;
    ...
    const projectionContainer = document.querySelector('#projection-chart').parentNode;
    
    // Remove existing summary if present
    const existingSummary = projectionContainer.querySelector('.projection-summary');
    if (existingSummary) {
    ...
    summaryDiv.className = 'projection-summary mt-4 p-3 bg-gray-50 rounded-lg border border-gray-200 text-sm';
    ...
    summaryDiv.innerHTML = `
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
            <div>
                <div>Expected Value after ${appState.investmentGoals.timeHorizon} years:</div>
                <div class="font-bold text-blue-600">$${Math.round(finalExpected).toLocaleString()}</div>
            </div>
            <div>
                <div>Target Goal: $${targetAmount.toLocaleString()}</div>
                <div class="font-bold ${targetMet ? 'text-green-600' : 'text-red-600'}">
...
function createRiskDialChart(riskProfile) {
    ...
    const riskLevels = ["Very Conservative", "Conservative", "Moderate", "Growth-Oriented", "Aggressive"];
